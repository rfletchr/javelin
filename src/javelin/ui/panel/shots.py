...
class ShotItem(QtGui.QStandardItem):
    ...
    def __init__(self, entity: dict, status_code_to_name: dict[str, str]):
        super().__init__()
        self.setEditable(False)
        self.setData(entity, ItemDataRole.UserRole)
        self.setData(entity["code"], ModelRoles.NameRole)

        self.setData(status_code_to_name[entity["sg_status_list"]], ModelRoles.StatusRole)
...
class ShotsController(BaseController):
            ...
            PromiseAll(
                [
                    self.db.find(
                        ...
                        fields=ShotItem.fields(),
                    ),
                    self.db.find(self, "Status", [], ["code", "name"]),
                ]
            )
    ...
    def onShotsFetched(self, results: tuple[list[dict], list[dict]]):
        entities, statuses = results
        status_code_to_name = {status["code"]: status["name"] for status in statuses}
        self.model.setItems([ShotItem(e, status_code_to_name) for e in entities])
